...
<%
    ...
    if(msgto.equals("allunitmembers"))
    {   psmt2=con.prepareStatement("select * from tbl_ads where ads_id=?");
        psmt2.setInt(1,userid);
        rs2=psmt2.executeQuery();
        if(rs2.next())
        {   
            psmt1=con.prepareStatement("select * from tbl_member where wardno=?");
    ...
    if(msgto.equals("unitadmins"))
    {   psmt2=con.prepareStatement("select * from tbl_ads where ads_id=?");
        psmt2.setInt(1,userid);
        rs2=psmt2.executeQuery();
        if(rs2.next())
        {   
            psmt1=con.prepareStatement("select * from tbl_unit where wardno=?");
            psmt1.setInt(1,rs2.getInt("wardno"));
            rs1=psmt1.executeQuery();
            while(rs1.next())
            {
    ...
    response.sendRedirect("../CAN/adssendmessage.jsp?msg=Successfully Send Message");
