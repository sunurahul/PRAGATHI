...
<%
    ...
    if(msgto.equals("allmembers"))
    {     
            psmt1=con.prepareStatement("select * from tbl_member where userid <> 0");
    ...
    if(msgto.equals("allads"))
    {   psmt2=con.prepareStatement("select * from tbl_ads");
        rs2=psmt2.executeQuery();
        while(rs2.next())
        {   
            
            psmt=con.prepareStatement("insert into tbl_messagedetails(toid,msgid,totype)"+"values(?,?,?)");
            psmt.setInt(1,rs2.getInt("ads_id"));
            psmt.setInt(2,msgid);
    ...
    if(msgto.equals("allunitadmins"))
    {   
        psmt1=con.prepareStatement("select * from tbl_unit");
        rs1=psmt1.executeQuery();
        while(rs1.next())
        {
        psmt=con.prepareStatement("insert into tbl_messagedetails(toid,msgid,totype)"+"values(?,?,?)");
        psmt.setInt(1,rs1.getInt("admin_id"));
        psmt.setInt(2,msgid);
        psmt.setString(3,"unitadmin");
        res=psmt.executeUpdate();
    ...
    response.sendRedirect("../Admin/adminsendmessage.jsp?msg=Successfully Send Message");
